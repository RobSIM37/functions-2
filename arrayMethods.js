...
// CODE HERE
const postTaxPrices = prices.map(elem => parseFloat((elem * 1.07).toFixed(2)));
...
// CODE HERE
const totalPopulation = populations.reduce((ack, elem)=>ack+elem);
...
// CODE HERE
const orderTotals = orders.map(order => parseFloat((order.price * (1 + order.tax)).toFixed(2)));
for (let i=0; i<orderTotals.length; i++) {
  let total = orderTotals[i];
  console.log(total);
}
...
// CODE HERE
let owner = "Bob"
let bobsTotal = purchases.reduce((runningTotal, currentItem) => currentItem.owner === owner ? runningTotal + currentItem.price : runningTotal + 0, 0)
